...
int main()
{
	...
	t = 1000000; //Кол-во экспериментов
	...
	vector<Vec3> par; //Вектор координат точек, спроецированных на поверхность куба со сферы

	//Проецирование равномерно расставленных точек с поверхности сферы на поверхность куба
	for (int i = 0; i < n; i++) {
		double cx, cy, cz;
		cx = 1; cy = 1; cz = 1;
	...
	//Кидание точки на поверхность куба
	for (int i = 0; i < t; i++) {
		int rng, znak;
		rng = 1 + rand() % 3;
		znak = 1 + rand() % 2;
	...
	//Вывод точки
	cout << endl << "Вывод точки: (" << pos[0].x << ", " << pos[0].y << ", " << pos[0].z << ")" << endl;
